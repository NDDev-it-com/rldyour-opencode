...
# ---------- Permission keys (v1.17.6 canonical set) ----------
...
def test_opencode_json_accepts_all_mode(tmp_path: Path) -> None:
    """v1.17.6 agent.mode accepts `all` (per built-in customize-opencode skill)."""
    cfg = tmp_path / "opencode.json"
    cfg.write_text(
...
def test_agent_mode_all_accepted(tmp_path: Path) -> None:
    """OpenCode v1.17.6 docs (https://opencode.ai/docs/agents) allow
