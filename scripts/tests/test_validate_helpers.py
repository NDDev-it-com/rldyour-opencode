...
# ---------- Permission keys (v1.17.7 canonical set) ----------


def test_canonical_permission_keys_has_expected_size() -> None:
    """v1.17.7 canonical set has 15 keys (verified via built-in customize-opencode skill)."""
...
def test_opencode_json_accepts_all_mode(tmp_path: Path) -> None:
    """v1.17.7 agent.mode accepts `all` (per built-in customize-opencode skill)."""
    cfg = tmp_path / "opencode.json"
    cfg.write_text(
...
def test_agent_mode_all_accepted(tmp_path: Path) -> None:
    """OpenCode v1.17.7 docs (https://opencode.ai/docs/agents) allow
