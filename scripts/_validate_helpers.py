...
# v1.17.6 canonical permission key set sourced from the built-in
# `customize-opencode` skill (`opencode debug skill` -> the `<built-in>`
# entry) and cross-validated against the v1.17.6 JSON Schema published
    ...
    for key in perm:
        if key not in CANONICAL_PERMISSION_KEYS:
            ...
            print(
                ...
                f"(canonical set for v1.17.6: {sorted_canon})"
            )
            errors += 1
    return errors
...
def validate_opencode_json(path: Path) -> int:
    ...
    if errors == 0 and (cfg.get("permission") or cfg.get("agent")):
        print(
            f"[OK] Permission keys conform to v1.17.6 canonical set "
...
def validate_agent(agent_md: Path) -> int:
    ...
    # OpenCode v1.17.6 supports mode: primary | subagent | all (default all).
    ...
    # Permission keys must come from the v1.17.6 canonical set.
