...
# v1.17.13 canonical permission key set sourced from the built-in
# `customize-opencode` skill (`opencode debug skill` -> the `<built-in>`
# entry) and cross-validated against the v1.17.13 JSON Schema published
# at https://opencode.ai/config.json (byte-identical to the v1.17.7
# snapshot; the key set is unchanged). Unknown keys are silently accepted
    ...
    for key in perm:
        if key not in CANONICAL_PERMISSION_KEYS:
            ...
            print(
                ...
                f"(canonical set for v1.17.13: {sorted_canon})"
            )
            errors += 1
    return errors
...
def validate_opencode_json(path: Path) -> int:
    ...
    for key in ("theme", "keybinds", "tui"):
        if key in cfg:
            print(
                f"[ERR] opencode.json must not contain legacy top-level {key!r}; "
                "use dedicated tui.json / OPENCODE_TUI_CONFIG for TUI state"
            )
            errors += 1
    ...
    if errors == 0 and (cfg.get("permission") or cfg.get("agent")):
        print(
            f"[OK] Permission keys conform to v1.17.13 canonical set "
...
def validate_agent(agent_md: Path) -> int:
    ...
    # OpenCode v1.17.13 supports mode: primary | subagent | all (default all).
    ...
    # Permission keys must come from the v1.17.13 canonical set.
