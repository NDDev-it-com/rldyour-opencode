...
2. **Figma context** (if available) — `figma-to-code` extracts frames, variables, components, layout data, assets, Code Connect hints via `mcp__figma__*`.
...
5. **Components** — shadcn MCP via `mcp__shadcn__*` (Browse / Search / Install) for primitives and registry blocks; ReactBits.dev only for purposeful motion or interactive effects.
6. **Implementation** — Serena-first inspection (`get_symbols_overview` → `find_symbol`), then code; never paste generated code blindly.
7. **Validation** — `design-validation` + `browser-validation` (Playwright `mcp__playwright__*` + Chrome DevTools `mcp__chrome-devtools__*`) for pixel-perfect, responsive, runtime, business-logic checks.
