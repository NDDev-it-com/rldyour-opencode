...
- Workflow-level `permissions:` block declares minimal scope (typically `contents: read`); job-level overrides only where strictly required (`security-events: write` for CodeQL, `contents: write` + `id-token: write` for release).
...
| `secret-scan.yml` | push + PR | Linux | gitleaks CLI release tarball with SHA256 verification and checkout fetch-depth: 0 |
