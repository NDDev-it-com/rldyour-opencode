...
| --- | --- | --- | --- | --- |
| OpenCode runtime/schema/package baseline | 1.17.18 | Adopted | `opencode-ai`, `@opencode-ai/plugin`, and `@opencode-ai/sdk` are pinned together at `1.17.18`; the real runtime resolver rewrites mismatched plugin packages to its own line, so a partial SDK-only bump is not accepted. The vendored config schema remains `references/opencode-config.schema.v1.17.18.json`. | `scripts/check_baseline_consistency.py`; `scripts/validate_opencode_schema.py` |
