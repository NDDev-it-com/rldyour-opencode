...
| --- | --- | --- | --- | --- |
| OpenCode runtime/schema/package baseline | 1.17.18 | Adopted | `opencode-ai`, `@opencode-ai/plugin`, and `@opencode-ai/sdk` are pinned to `1.17.18`; npm package metadata is the authoritative latest channel, GitHub Releases are informational, and the vendored config schema is `references/opencode-config.schema.v1.17.18.json` fetched from the live `https://opencode.ai/config.json`. The `1.17.18` schema is byte-identical to the previously vendored `v1.17.7`/`v1.17.13`/`v1.17.14` snapshots (SHA-256 `57c02429`); the filename tracks the current runtime baseline. | `scripts/check_baseline_consistency.py`; `scripts/validate_opencode_schema.py` |
