...
3. Bootstrap agent-only context before treating `AGENTS.md`, `.serena/*`, `.opencode/*`, or similar files as missing. Restore an existing `fullrepo`, publish local agent-only files when no `fullrepo` exists, install `.git/info/exclude`, and remove tracked agent-only files from the current branch index when migration is needed.
