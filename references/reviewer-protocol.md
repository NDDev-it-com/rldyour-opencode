...
- No model override - reviewers inherit the top-level `model` from `opencode.json` (currently `moonshotai/kimi-k2.7-code`). This ensures reviewers always run on the user's selected model, not a hardcoded fallback. Cost/latency trade-offs are the user's responsibility via model selection.
